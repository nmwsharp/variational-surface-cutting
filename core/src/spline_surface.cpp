...
Vector3 BezierPatch<Vector3>::tangentU(const PatchParam& point)
{
   ...
   return dBu0 * ( Bv0 * c[0][0] + Bv1 * c[0][1] + Bv2 * c[0][2] + Bv3 * c[0][3] )
        + dBu1 * ( Bv0 * c[1][0] + Bv1 * c[1][1] + Bv2 * c[1][2] + Bv3 * c[1][3] )
        + dBu2 * ( Bv0 * c[2][0] + Bv1 * c[2][1] + Bv2 * c[2][2] + Bv3 * c[2][3] )
...
Vector3 BezierPatch<Vector3>::tangentV(const PatchParam& point)
{
   ...
   array4x4<Vector3>& c = controlPoints;
   return dBv0 * ( Bu0 * c[0][0] + Bu1 * c[1][0] + Bu2 * c[2][0] + Bu3 * c[3][0] )
        + dBv1 * ( Bu0 * c[0][1] + Bu1 * c[1][1] + Bu2 * c[2][1] + Bu3 * c[3][1] )
        + dBv2 * ( Bu0 * c[0][2] + Bu1 * c[1][2] + Bu2 * c[2][2] + Bu3 * c[3][2] )
...
Vector3 GregoryPatchTri<Vector3>::tangentU(const PatchParam& point)
{
   ...
   return 3.*(u*(2.*e0m - 3.*e0m*u + p0*u) -
          3.*e2m*v + 4.*F2*v +
          u*(-4.*e0m + 6.*e2m + 8.*F0 - 16.*F2 + 3.*(e0m + e0p - e2m - 4.*F0 + 4.*F2)*u)*v +
          2.*(-e1p + 2.*e2m + 2.*F1 - 4.*F2 + (e0m + e0p + e1m + e1p - 2.*(e2m + 2.*(F0 + F1 - 2.*F2)))*u)*((v)*(v)) +
...
Vector3 GregoryPatchTri<Vector3>::tangentV(const PatchParam& point)
{
   ...
   // making the approximation that the face points are fixed
   // (This expression was automatically generated using Mathematica)
   return 3*(u*(-3*e2p + 4*F2 - 2*e0m*u + 4*(e2p + F0 - 2*F2)*u + (e0m + e0p - e2p - 4*F0 + 4*F2)*((u)*(u))) +
             2*e1p*v + 2*u*(-2*e1p + 3*e2p + 4*F1 - 8*F2 + (e0m + e0p + e1m + e1p - 2*(e2p + 2*(F0 + F1 - 2*F2)))*u)*v +
             (-3*e1p + p1 + 3*(e1m + e1p - e2p - 4*F1 + 4*F2)*u)*((v)*(v)) -
             p2*((-1 + u + v)*(-1 + u + v)) -
...
Vector3 GregoryPatchQuad<Vector3>::tangentU(const PatchParam& point)
{
   ...
   // return 9*(1 - u)*sqr(u)*(-(e1m*sqr(-1 + v)) + e2p*sqr(v) + (f1m*sqr(-1 + v)*v)/(1 - u + v) + ((f2p*(-1 + u) + f2m*(-1 + v))*(-1 + v)*sqr(v))/sqr(-2 + u + v) - (2*(f2p*(-1 + u) + f2m*(-1 + v))*(-1 + v)*v)/(-2 + u + v) - ((f2p*(-1 + u) + f2m*(-1 + v))*sqr(v))/(-2 + u + v) - (f2m*(-1 + v)*sqr(v))/(-2 + u + v) - (sqr(-1 + v)*v*(f1p - f1p*u + f1m*v))/sqr(1 - u + v) + (sqr(-1 + v)*(f1p - f1p*u + f1m*v))/(1 - u + v) + (2*(-1 + v)*v*(f1p - f1p*u + f1m*v))/(1 - u + v)) - 3*cube(-1 + u)*(-(p0*sqr(-1 + v)) + v*(2*e3p - 3*e3p*v + p3*v) + e0m*(1 - 4*v + 3*sqr(v))) + 3*cube(u)*(-(p1*sqr(-1 + v)) + v*(2*e2m - 3*e2m*v + p2*v) + e1p*(1 - 4*v + 3*sqr(v))) + 9*sqr(-1 + u)*u*(-(e0p*sqr(-1 + v)) + (f0p*u*sqr(1 + u - v)*(-1 + v)*(2*sqr(v) + u*(-1 + 3*v)) + v*(f0m*sqr(1 + u - v)*(-1 + v)*(v*(-1 + 3*v) + u*(-2 + 4*v)) + sqr(u + v)*(f3p*u*(u*(2 - 3*v) + 2*sqr(-1 + v)) + e3m*sqr(1 + u - v)*v + f3m*(-1 + v)*(-2 + 5*v - 3*sqr(v) + u*(-2 + 4*v)))))/(sqr(1 + u - v)*sqr(u + v)));
}
...
Vector3 GregoryPatchQuad<Vector3>::tangentV(const PatchParam& point)
{
   ...
   // // Alt: exact derivative of rational expression (computed in Mathematica)
   // return 3.*((3.*(-1. + u)*sqr(u)*sqr(-1. + v)*sqr(v)*(-((f2m - f2p)*sqr(1. - u + v)) - f1m*sqr(-2. + u + v) + f1p*sqr(-2. + u + v)))/(sqr(1. - u + v)*sqr(-2. + u + v)) - (3.*sqr(-1. + u)*u*sqr(-1. + v)*sqr(v)*(f0m*sqr(1. + u - v) - f0p*sqr(1. + u - v) + (f3m - f3p)*sqr(u + v)))/(sqr(1. + u - v)*sqr(u + v)) + 2.*(1. - u)*u*(-(e1m*cube(-1. + v)) + e2p*cube(v) - (3.*(f2p*(-1. + u) + f2m*(-1. + v))*(-1. + v)*sqr(v))/(-2. + u + v) + (3.*sqr(-1. + v)*v*(f1p - f1p*u + f1m*v))/(1. - u + v)) - sqr(u)*(-(e1m*cube(-1. + v)) + e2p*cube(v) - (3.*(f2p*(-1. + u) + f2m*(-1. + v))*(-1. + v)*sqr(v))/(-2. + u + v) + (3.*sqr(-1. + v)*v*(f1p - f1p*u + f1m*v))/(1. - u + v)) + sqr(-1. + u)*(-(e0p*cube(-1. + v)) + e3m*cube(v) + (3.*sqr(-1. + v)*v*(f0p*u + f0m*v))/(u + v) - (3.*(-1. + v)*sqr(v)*(f3m + f3p*u - f3m*v))/(1. + u - v)) - 2.*(1. - u)*u*(-(e0p*cube(-1. + v)) + e3m*cube(v) + (3.*sqr(-1. + v)*v*(f0p*u + f0m*v))/(u + v) - (3.*(-1. + v)*sqr(v)*(f3m + f3p*u - f3m*v))/(1. + u - v)) + sqr(u)*(-(p1*cube(-1. + v)) + v*(3.*e1p*sqr(-1. + v) + v*(3.*e2m - 3.*e2m*v + p2*v))) - sqr(-1. + u)*(-(p0*cube(-1. + v)) + v*(3.*e0m*sqr(-1. + v) + v*(3.*e3p - 3.*e3p*v + p3*v))));
}
...
// Differential geometry (for T=Vector3 only) --- these methods just find the patch containing the query point and then call the correpsonding method from the patch.
template<> Vector3 SplineSurface<Vector3>::position(const PatchPoint& p) { return evaluate(p); } 
template<> Vector3 SplineSurface<Vector3>::normal  (const PatchPoint& p)     { SplinePatchPtr<Vector3> patch = getPatch(p.face); return patch->normal  (p.coords); } 
template<> Vector3 SplineSurface<Vector3>::tangentU(const PatchPoint& p)     { SplinePatchPtr<Vector3> patch = getPatch(p.face); return patch->tangentU(p.coords); } 
template<> Vector3 SplineSurface<Vector3>::tangentV(const PatchPoint& p)     { SplinePatchPtr<Vector3> patch = getPatch(p.face); return patch->tangentV(p.coords); }
//template<> Vector3 SplineSurface<Vector3>::pushForward(const SplineTangent& tangent) { SplinePatchPtr<Vector3> patch = getPatch(p.face); return patch->tangentV(p.coords); }
