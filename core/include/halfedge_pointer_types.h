...
// === Forward declare iterator set types (pointers may need to return these to support range-based for loops)
...
class EdgePtr {
   ...
   public:
      ...
      // Remeshing
      bool flip(); // flips a triangle pair, returning false if the faces aren't triangles or the edge is on the boundary
...
struct Triangle {
   VertexPtr vertex[3];
   VertexPtr& operator[]( size_t index ) {
      return vertex[index];
   }
};
