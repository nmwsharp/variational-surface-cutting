...
inline T Geometry<T>::position(DualVertexPtr v)
{
   ...
   throw std::domain_error("Dual vertex position currently implemented only for barycentric and circumcentric dual");
}
...
inline T Geometry<T>::midpoint(EdgePtr e) {
   return .5 * ( p[e.halfedge().vertex()] +
                 p[e.halfedge().twin().vertex()] );
}
...
inline double Geometry<T>::length(DualEdgePtr e) {
   ...
   if( dualType != DualType::Circumcentric )
   {
      throw std::domain_error("Dual edge length currently implemented only for circumcentric dual");
   }
...
inline double Geometry<T>::area(DualFacePtr f) {
   ...
   if( dualType != DualType::Barycentric )
   {
      throw std::domain_error("Dual face area currently implemented only for barycentric dual");
   }
...
inline double Geometry<T>::angularCoordinate(HalfedgePtr h) {
    ...
    return 2*PI - coord; // we want the CCW value, but we just measured the CW value (due to the direction orbited by twin->next)
}
...
template<>
inline double Geometry<Spherical>::cotanWeight(EdgePtr e, bool faceAreaWeighted) {
...
inline Vector3 Geometry<Spherical>::normal(FacePtr f) {
   ...
   Vector3 pi = p[h.vertex()]; h = h.next();
   Vector3 pj = p[h.vertex()]; h = h.next();
   ...
   return thetaIJ * unit( cross( pj, pi )) +
          thetaJK * unit( cross( pk, pj )) +
...
template<class T>
void Geometry<T>::getPrincipalDirections( VertexData<Complex>& principalDirections, HalfedgeData<double>& angularCoordinates) {

    principalDirections = VertexData<Complex>(&mesh);

    // NOTE: This logic is duplicated here and in the per-vertex method (to use a cached coordinate array)
...
template<class T>
void Geometry<T>::getDualVertexPositions( DualVertexData<T>& dualVertexPosition ) {
...
template<class T>
void Geometry<T>::getEdgeCotanWeights( EdgeData<double>& edgeCotanWeight, bool faceAreaWeighted ) {
...
template<typename T, typename G>
GC::SparseMatrix<T> vertexMassMatrix(Geometry<G> *geometry, VertexData<size_t> index)
{
    if(geometry->dualType != DualType::Barycentric) {
        throw std::domain_error("Vertex mass matrix currently implemented only for barycentric dual");
    }
...
template<typename T, typename G>
GC::SparseMatrix<T> faceMassMatrix(Geometry<G> *geometry, FaceData<size_t> index)
{
    HalfedgeMesh& mesh(geometry->mesh);
    int nF = mesh.nFaces();
