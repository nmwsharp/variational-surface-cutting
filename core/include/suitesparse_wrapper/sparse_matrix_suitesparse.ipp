...
namespace GC
{
    ...
    public:
        ...
        // optionally specify a (positive) number of interior values for block reordering
    ...
    Eigen::SparseMatrix<T> SparseMatrix<T>::toEigen( void )
    {
        ...
        // into a flat list of triplets, then calling Eigen::SparseMatrix<T>::setFromTriplets()
   ...
   SparseMatrix<T> SparseMatrix<T> :: operator-( const SparseMatrix<T>& B ) const
   {
    ...
    {
        assert( row >= 0 ); assert( row < m );
        assert( col >= 0 ); assert( col < n );
    ...
    template<typename T>
    SparseMatrix<T> SparseMatrix<T> :: sub( size_t r0, size_t r1, size_t c0, size_t c1 ) const
    {
        // check bounds
    ...
    template<typename T>
    void SparseMatrix<T> :: setSubMatrix( size_t r0, size_t r1, size_t c0, size_t c1, const SparseMatrix<T>& B )
    {
        // check bounds
        assert( 0 <= r0 && r0 <= r1 && r1 < m );
        assert( 0 <= c0 && c0 <= c1 && c1 < n );
    ...
    void SparseMatrix<T> :: increment( size_t r0, size_t c0, const Matrix3x3& B )
    {
        ...
        A(r0+0,c0+0) += B(0,0); A(r0+0,c0+1) += B(0,1); A(r0+0,c0+2) += B(0,2);
        A(r0+1,c0+0) += B(1,0); A(r0+1,c0+1) += B(1,1); A(r0+1,c0+2) += B(1,2);
        A(r0+2,c0+0) += B(2,0); A(r0+2,c0+1) += B(2,1); A(r0+2,c0+2) += B(2,2);
    }
    ...
    T rayleighQuotient( const SparseMatrix<T>& A,
                        ...
                        const  DenseMatrix<T>& x )
    {
        return (x.transpose()*(A*x))(0) / (x.transpose()*(B*x-E*(E.transpose()*x)))(0);
    }
                        ...
                        const SparseMatrix<U>& B )
    {
        ...
        eA = A.begin(); endA = A.end();
        eB = B.begin(); endB = B.end();

        while( eA != endA && eB != endB )
        {
            if( eA->first.row != eB->first.row ||
                eA->first.col != eB->first.col )
            {
            return false;
            }
