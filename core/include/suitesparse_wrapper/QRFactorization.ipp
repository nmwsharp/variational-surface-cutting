...
namespace GC
{
   ...
   void QRFactorization<T> :: clear( void )
   {
      if( factor )
      {
         SuiteSparseQR_free<typename QRFactorization<T>::AUTO_ENTRYTYPE>( &factor, context );
         factor = NULL;
      }
   ...
   template <class T>
   SuiteSparseQR_factorization<typename QRFactorization<T>::AUTO_ENTRYTYPE>* QRFactorization<T> :: get( void )
   {
      if( !validSymbolic )
      {
         build();
   ...
   void QRFactorization<T> :: build( void )
   {
      ...
      START_TIMING(sym)
      factor = SuiteSparseQR_symbolic<typename QRFactorization<T>::AUTO_ENTRYTYPE>( ordering, allow_tol, Ac, context );
      std::cout << "[qr] symbolic: " << pretty_time(FINISH_TIMING(sym)) << std::endl;
   ...
   void QRFactorization<T> :: update( void )
   {
      ...
      START_TIMING(num)
      SuiteSparseQR_numeric<typename QRFactorization<T>::AUTO_ENTRYTYPE>( tol, Ac, factor, context );
