...
namespace GC
{
    ...
    void solve( SparseMatrix<T>& A,
                ...
                DenseMatrix<T>  b )
    {
        SuiteSparseQR_factorization<typename QRFactorization<T>::AUTO_ENTRYTYPE>* factorization = A.factors.qr.get();
        DenseMatrix<T> y;

        START_TIMING(solve)

        // solve y = R'\(E'*b)
        y = SuiteSparseQR_solve( SPQR_RTX_EQUALS_ETB, factorization, b.to_cholmod(), context );
                      ...
                      DenseMatrix<T>  b )
    {
        ...
        START_TIMING(solve)
        umfpack_dl_solve( UMFPACK_A, Ap, Ai, Ax, (double*) &x(0), (double*) &b(0), factorization, NULL, NULL );
                            ...
                            DenseMatrix<T>  b )
    {
        ...
        // x = SuiteSparseQR<typename QRFactorization<T>::AUTO_ENTRYTYPE>( A.to_cholmod(), b.to_cholmod(), context );
        x = SuiteSparseQR<typename QRFactorization<T>::AUTO_ENTRYTYPE>( aa, bb, context );
                                ...
                                int maxIterations )
    {
        ...
        cout << "[pcg] achieved a residual of " << relativeResidual << " after " << iter << " iterations" << endl;
    }
    ...
    void SparseFactor<T>::print() const
    {
        ...
        else
        {
            ...
            {
                for( int col = Lsuper[node]; col < Lsuper[node+1]; col++ ) // iterate over columns in this supernode
                {
                    for( int k = Lpi[node]; k < Lpi[node+1]; k++ ) // iterate over nonzero row indices for this supernode
                    {
